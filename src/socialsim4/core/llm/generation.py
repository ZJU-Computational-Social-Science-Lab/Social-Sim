...
JSON only, no other text."""
    ...
    # Check for empty response - likely Ollama/local LLM issue
    if not response or not response.strip():
        raise RuntimeError(
            f"LLM returned empty response for archetype '{attrs_str}'. "
            f"This usually indicates:\n"
            f"  1. Ollama/local model crashed or ran out of memory\n"
            f"  2. Model is still loading (try again in a few seconds)\n"
            f"  3. Request timed out\n"
            f"Check your LLM server logs for details."
        )
    ...
    if not json_match:
        raise RuntimeError(
            f"No JSON found in LLM response for archetype '{attrs_str}'. "
            f"Response was: {cleaned[:200]}"
        )
    ...
    if "description" not in parsed or not isinstance(parsed["description"], str):
        raise RuntimeError(
            f"Missing or invalid 'description' for archetype '{attrs_str}'. "
            f"Got keys: {list(parsed.keys())}"
        )
    if "roles" not in parsed or not isinstance(parsed["roles"], list) or len(parsed["roles"]) == 0:
        raise RuntimeError(
            f"Missing or invalid 'roles' for archetype '{attrs_str}'. "
            f"Got: {parsed.get('roles')}"
        )
    ...
    for i, r in enumerate(parsed["roles"]):
        if not isinstance(r, str):
            raise RuntimeError(
                f"Role {i} must be a string for archetype '{attrs_str}', "
