...
JSON only, no explanation."""
        ...
        try:
            ...
            logger.info(f"Summarize LLM response: {response[:200]}...")
...
JSON only, no explanation."""
        ...
        try:
            ...
            logger.info(f"Validated {len(result)} suggestions from {len(suggestions)} total")
            return result
        except Exception as e:
            logger.exception("Failed to generate suggestions: %s", e)
            # Fallback suggestion
            logger.info("Using fallback suggestion")
            return [{
