...
- You defect, they defect: {params['defect_penalty']} points"""
    ...
    def _build_params_section(self) -> str:
        ...
        elif scenario_id == "xihu_yilianbao":
            intervention_arm = params.get("intervention_arm", "")
            decision_focus = params.get("decision_focus", "")
            household_burden = params.get("household_medical_burden", "")
            prior_attitude = params.get("prior_insurance_attitude", "")
            government_trust = params.get("government_trust")
            personal_account_cue = params.get("personal_account_cue", "")
            deadline_cue = params.get("deadline_cue", "")

            intervention_map = {
                "A0": "Agents receive no policy information beyond their existing beliefs.",
                "A0 控制组": "Agents receive no policy information beyond their existing beliefs.",
                "A1": "Agents receive the full official policy text with detailed clauses, prices, and coverage responsibilities.",
                "A1 官方复杂材料": "Agents receive the full official policy text with detailed clauses, prices, and coverage responsibilities.",
                "A2": "Agents receive a simplified visual summary that foregrounds eligibility, premium, coverage ceiling, and service highlights.",
                "A2 简明图文": "Agents receive a simplified visual summary that foregrounds eligibility, premium, coverage ceiling, and service highlights.",
                "A3": "Agents receive positive cases showing both household relief and personal reimbursement gains after enrollment.",
                "A3 家庭+个人收益": "Agents receive positive cases showing both household relief and personal reimbursement gains after enrollment.",
                "A4": "Agents receive a loss-framed case emphasizing how much one person can lose by staying uninsured.",
                "A4 个人损失": "Agents receive a loss-framed case emphasizing how much one person can lose by staying uninsured.",
                "A5": "Agents receive a loss-framed family case emphasizing the burden transferred to relatives when nobody enrolled.",
                "A5 家庭损失": "Agents receive a loss-framed family case emphasizing the burden transferred to relatives when nobody enrolled.",
                "A6": "Agents receive a gain-framed personal case emphasizing how much one enrollee can save.",
                "A6 个人收益": "Agents receive a gain-framed personal case emphasizing how much one enrollee can save.",
                "A7": "Agents first see negative news about a competing million-yuan medical product, then a government-supervised positive message about Xihu Yilianbao.",
                "A7 竞品负面+官方正面": "Agents first see negative news about a competing million-yuan medical product, then a government-supervised positive message about Xihu Yilianbao.",
                "A8": "Agents receive a short video-style explanation that compresses the policy into an easier narrative format.",
                "A8 视频信息": "Agents receive a short video-style explanation that compresses the policy into an easier narrative format.",
            }
            focus_map = {
                "只考虑自己": "Agents are mainly weighing whether to insure themselves.",
                "优先父母": "Agents are mainly weighing whether to insure their parents.",
                "优先子女": "Agents are mainly weighing whether to insure their children.",
                "家庭统筹": "Agents are balancing self, parent, and child coverage as one linked household decision.",
            }
            burden_map = {
                "低": "Recent medical spending is not very salient in the household.",
                "中等": "Medical spending is noticeable but still manageable for the household.",
                "高": "Recent illness or hospitalization has made medical expenses highly salient for the household.",
            }
            attitude_map = {
                "保守怀疑": "Most agents begin from a skeptical view of supplementary commercial insurance.",
                "中性观望": "Most agents begin in a wait-and-see position toward supplementary insurance.",
                "普遍积极": "Most agents begin with a broadly positive attitude toward supplementary insurance.",
                "分化": "Agents begin with mixed prior beliefs about supplementary insurance and government-backed products.",
            }

            if intervention_arm:
                lines.append(intervention_map.get(intervention_arm, f"Intervention package: {intervention_arm}."))
            if decision_focus:
                lines.append(focus_map.get(decision_focus, f"Decision focus: {decision_focus}."))
            if household_burden:
                lines.append(burden_map.get(household_burden, f"Household medical burden: {household_burden}."))
            if prior_attitude:
                lines.append(attitude_map.get(prior_attitude, f"Prior insurance attitude: {prior_attitude}."))
            if government_trust is not None:
                trust_value = float(government_trust)
                if trust_value <= 0.33:
                    trust_label = "low"
                elif trust_value <= 0.66:
                    trust_label = "moderate"
                else:
                    trust_label = "high"
                lines.append(f"Baseline trust in government supervision of the product is {trust_label}.")
            if personal_account_cue == "提示可用":
                lines.append("Agents are reminded that eligible households can use medical savings account balances to help pay the premium.")
            elif personal_account_cue == "不提示":
                lines.append("Agents are not reminded about medical savings account payment support.")
            if deadline_cue == "提示投保期":
                lines.append("Agents are reminded that enrollment is available only during a limited campaign window.")
            elif deadline_cue == "不提示":
                lines.append("Agents are not reminded about the enrollment deadline.")
