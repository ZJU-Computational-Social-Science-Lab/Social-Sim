...
class ExperimentRunner:
    ...
    def _get_neighbors(self, agent_name: str) -> List[str]:
        """Return neighbors from either edge-list or adjacency-list graph config."""
        graph = self.scene_state.get("graph", {})
        if not graph:
            return []
        if "edges" in graph:
            edges = graph.get("edges", [])
            return [b for a, b in edges if a == agent_name] + [a for a, b in edges if b == agent_name]
        return list(graph.get(agent_name, []))
    ...
    def _generate_coordination_feedback(
        ...
        """
        neighbors = self._get_neighbors(agent_name)
    ...
    async def _prompt_agent(self, agent: ExperimentAgent, round_num: int) -> ActionResult:
        ...
        neighbor_context = ""
        neighbors = self._get_neighbors(agent.name)
        if neighbors:
        ...
        if self.information_model and self.information_model.scope_type in ("neighborhood", "neighbor"):
            ...
            if graph:
                ...
                if "edges" in graph:
                    ...
                    for agent_name, neighbors in graph.items():
                        neighbor_map[agent_name] = list(neighbors)
