 ...
 * Opt-in only — requires SOCIALSIM_TEST_REAL_LLM=1 plus a running
 ...
 *
 * Required env vars:
 *   SOCIALSIM_TEST_REAL_LLM=1
 *   SOCIALSIM_TEST_LLM_PROVIDER=ollama  (default)
 *   SOCIALSIM_TEST_LLM_MODEL=<model>    (e.g. qwen3:4b)
 *
 * Optional:
...
const REAL_LLM = process.env.SOCIALSIM_TEST_REAL_LLM === '1';
const LLM_PROVIDER = process.env.SOCIALSIM_TEST_LLM_PROVIDER || 'ollama';
const LLM_MODEL = process.env.SOCIALSIM_TEST_LLM_MODEL || '';

test.beforeEach(async ({ locale }) => {
  test.skip(!REAL_LLM,
    'Set SOCIALSIM_TEST_REAL_LLM=1 to enable real-LLM E2E tests');
  test.skip(LLM_PROVIDER !== 'ollama',
    'Real-LLM E2E tests require Ollama. Set SOCIALSIM_TEST_LLM_PROVIDER=ollama');
  test.skip(!LLM_MODEL,
    'Set SOCIALSIM_TEST_LLM_MODEL to a locally installed Ollama model');
  test.skip(locale !== 'en',
