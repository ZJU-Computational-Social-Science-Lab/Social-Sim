...
def test_xihu_yilianbao_game_config_uses_neighbor_actions_and_prompt_context():
    config = ExperimentConfig(
        ...
        parameters={
            "intervention_arm": "A7",
            "decision_focus": "家庭统筹",
            "household_medical_burden": "高",
            "prior_insurance_attitude": "分化",
            "government_trust": 0.8,
            "personal_account_cue": "提示可用",
            "deadline_cue": "提示投保期",
        },
    )
    ...
    assert game_config.grouping_mode == "neighbor"
    assert game_config.payoff_type == "none"
    assert "competing million-yuan medical product" in game_config.payoff_summary
    assert "medical savings account balances" in game_config.payoff_summary
    assert "limited campaign window" in game_config.payoff_summary
