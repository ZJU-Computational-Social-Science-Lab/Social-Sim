...
class TestChatDispatch:
    ...
    def test_openai_dispatch(self):
        ...
        with patch("socialsim4.core.llm.client._get_openai", return_value=_openai_dict(openai_chat=chat_fn)):
    ...
    def test_gemini_dispatch(self):
        ...
        with patch("socialsim4.core.llm.client._get_gemini", return_value=_gemini_dict(gemini_chat=chat_fn)):
    ...
    def test_ollama_dispatch(self):
        ...
        with patch("socialsim4.core.llm.client._get_ollama", return_value=_ollama_dict(ollama_chat=chat_fn)):
    ...
    def test_mock_dispatch(self):
        with patch("socialsim4.core.llm.client._get_openai", return_value={
...
class TestCompletionDispatch:
    ...
    def test_openai_completion(self):
        ...
        with patch("socialsim4.core.llm.client._get_openai", return_value=_openai_dict(openai_completion=comp_fn)):
            c = LLMClient(LLMConfig(dialect="openai", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
    ...
    def test_gemini_completion(self):
        ...
        with patch("socialsim4.core.llm.client._get_gemini", return_value=_gemini_dict(gemini_completion=comp_fn)):
            c = LLMClient(LLMConfig(dialect="gemini", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
            result = c.completion("prompt")
    ...
    def test_ollama_completion(self):
        ...
        with patch("socialsim4.core.llm.client._get_ollama", return_value=_ollama_dict(ollama_completion=comp_fn)):
            c = LLMClient(LLMConfig(dialect="ollama", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
            result = c.completion("prompt")
...
class TestEmbeddingDispatch:
    ...
    def test_openai_embedding(self):
        ...
        with patch("socialsim4.core.llm.client._get_openai", return_value=_openai_dict(openai_embedding=emb_fn)):
            c = LLMClient(LLMConfig(dialect="openai", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
    ...
    def test_gemini_embedding(self):
        ...
        with patch("socialsim4.core.llm.client._get_gemini", return_value=_gemini_dict(gemini_embedding=emb_fn)):
            c = LLMClient(LLMConfig(dialect="gemini", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
            result = c.embedding("text")
    ...
    def test_ollama_embedding(self):
        ...
        with patch("socialsim4.core.llm.client._get_ollama", return_value=_ollama_dict(ollama_embedding=emb_fn)):
            c = LLMClient(LLMConfig(dialect="ollama", api_key="k", model="m"))
            c.max_retries = 0
            c.retry_backoff_s = 0.0
            result = c.embedding("text")
